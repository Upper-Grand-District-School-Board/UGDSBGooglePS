...
$params = @{
  SourcePath = "$PSScriptRoot\Source\UGDSBGooglePS.psd1"
  Version = $global:Version
  CopyPaths = @("$PSScriptRoot\Source\UGDSBGooglePS.nuspec")
  OutputDirectory = $moduleDir 
  UnversionedOutputDirectory = $true
}
Build-Module @params
$path = (Get-Module -ListAvailable UGDSBGooglePS | Where-Object {$_.Path -like "*$($global:Version)*"}).path
if($path){
  $directory = (Get-Item $Path).DirectoryName
  $copyFrom = $moduleDir
  $filesToCopy = Get-ChildItem -Path $copyFrom -recurse -Filter "*GooglePS.ps*"
